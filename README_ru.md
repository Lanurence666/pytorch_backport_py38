...
- **NumPy** (совместимая с Python 3.8 версия, например `numpy==1.24.4`)

### Шаги сборки (режим редактирования / разработки)
...
### Шаги сборки (Wheel-пакет)

Для сборки распространяемого `.whl` пакета:

```bash
# 1. Клонируйте и настройте среду (те же шаги 1-4, что и выше)

# 2. Соберите wheel
pip wheel --no-build-isolation -w dist .

# 3. Wheel будет в каталоге dist/:
#    dist/torch-2.13.0a0+git03855a7-cp38-cp38-win_amd64.whl
```

> **Совет:** Если при линковке `torch_cpu.dll` возникают ошибки памяти компоновщика (LNK1102), уменьшите `MAX_JOBS` до 1:
> ```bash
> set MAX_JOBS=1
> ```
...
- Установите `MAX_JOBS=2` (или `1` для систем с малым объёмом памяти) для предотвращения ошибок памяти компоновщика (LNK1102) при линковке `torch_cpu.dll`
- Полная сборка занимает примерно 2-4 часа на современной машине
- Результирующий wheel-пакет занимает приблизительно 160 МБ
- Для сборки без CUDA установите `set USE_CUDA=0`
