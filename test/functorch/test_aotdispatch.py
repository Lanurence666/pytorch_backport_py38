...
def forward(self, primals_1):
    ...
    def _compile_autocast(self, device, *, forward_autocast):
        with torch.library._scoped_library("mylib", "FRAGMENT") as m:
            ...
            if forward_autocast:
                with torch.amp.autocast(device), torch._dynamo.config.patch(recompile_limit=999):
...
def forward(self, primals_1, tangents_1):
    ...
    def test_functionalized_rng_codegen_unit_return_new_outs(self):
        ...
        with mock_patch( "torch._prims_common.CUDARngStateHelper.get_torch_state_as_tuple", mock_get_rng_state, ), mock_patch( "torch._prims_common.CUDARngStateHelper.set_new_offset", mock_set_offset, ):
