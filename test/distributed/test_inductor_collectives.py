...
class TestCollectivesMultiProc(DynamoDistributedMultiProcTestCase):
    ...
    def test_all_to_all_recompute_is_always_banned(self, override_with_ac):
        ...
        with _dynamo_dist_per_rank_init(self.rank, self.world_size), torch._dynamo.config.patch( dynamic_shapes=True, capture_dynamic_output_shape_ops=True, capture_scalar_outputs=True, ), torch.library._scoped_library("custom_ns", "FRAGMENT") as lib:
            lib.define(
...
class TestCollectivesInductor(DynamoDistributedSingleProcTestCase):
    ...
    def test_all_gather_bucket(self, bucket_mode):
        ...
        with torch._inductor.config.patch( { "bucket_all_gathers_bucket_mode": bucket_mode, "reorder_for_compute_comm_overlap": False, "runtime_estimations_mms_benchmark": True, } ), torch._inductor.config_comms.patch( { "runtime_estimations_align_across_all_distributed_ranks": True, } ), fresh_inductor_cache():
            compiled = torch.compile(func)
            code = run_and_get_triton_code(compiled, *inputs, **self.get_world_trs())
        # NOTE: The first return value should be the output of the first wait_tensor.
        # We want to make sure no unnecessary copy is made.
