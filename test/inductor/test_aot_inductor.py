...
class AOTInductorTestsTemplate:
    ...
    def test_constant_folding_with_update(self):
        ...
        example_inputs = (torch.randn(4, 4, device=self.device),)
        with torch.no_grad(), config.patch( { "always_keep_tensor_constants": True, "aot_inductor.use_runtime_constant_folding": True, } ):
            model = Model(self.device)
    ...
    def test_buffer_mutation_and_force_mmap_weights(self):
        ...
        with config.patch({"freezing": True, "aot_inductor.force_mmap_weights": True}), torch.no_grad():
            self.check_model(model, example_inputs)
    ...
    def test_aoti_user_defined_triton_kernel_profiling(self):
        ...
        with config.patch({"cpp.enable_kernel_profile": True}), torch.profiler.profile( record_shapes=True, activities=[ torch.profiler.ProfilerActivity.CPU, getattr(torch.profiler.ProfilerActivity, GPU_TYPE.upper()), ], ) as prof:
            self.check_model(Model(), example_inputs)
    ...
    def test_so_without_weight(self):
        ...
        example_inputs = (a,)
        with torch.no_grad(), config.patch( { "always_keep_tensor_constants": True, "aot_inductor.package_constants_in_so": True, } ):
            so_path = AOTIRunnerUtil.legacy_compile(
                model=model,
                example_inputs=example_inputs,
            )
    ...
    def test_weight_on_disk_legacy(self):
        ...
        with torch.no_grad(), config.patch( { "always_keep_tensor_constants": True, "aot_inductor.package_constants_in_so": False, "aot_inductor.package_constants_on_disk_format": "pickle_weights", "aot_inductor.package": True, } ):
    ...
    def test_rocm_triton_autotuning(self):
        ...
        with torch.no_grad(), config.patch( { "triton.autotune_with_sample_inputs": True, "aot_inductor.allow_stack_allocation": self.allow_stack_allocation, "aot_inductor.use_minimal_arrayref_interface": self.use_minimal_arrayref_interface, } ):
...
class AOTInductorLoggingTest(LoggingTestCase):
    ...
    def test_shape_env_reuse_zero_consts_use_consts_asm_false(self, records):
        ...
        ep = export(Foo(), inputs, dynamic_shapes=dynamic_shapes, strict=False)
        with torch.no_grad(), config.patch({"aot_inductor.use_consts_asm_build": False}):
            torch._inductor.aot_compile(ep.module(), inputs)
        self.assertEqual([r.msg == "create_env" for r in records].count(True), 1)
