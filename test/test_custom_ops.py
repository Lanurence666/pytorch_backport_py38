...
class TestOpProfiles(TestCase):
    ...
    def test_fake_registration(self):
        ...
        with torch.library._scoped_library("mylib", "FRAGMENT") as lib:
            ...
            with self.assertRaisesRegex( torch._subclasses.fake_tensor.UnsupportedOperatorException, "mylib.foo2.default", ), fm:
                torch.ops.mylib.foo2(t1, t2)

            with torch._library.fake_profile.unsafe_generate_fake_kernels(op_profiles), fm:
                torch.ops.mylib.foo2(t1, t2)
