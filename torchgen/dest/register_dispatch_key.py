...
class RegisterDispatchKey:
    ...
    class_method_name: Union[str, None]
  ...
  c10::impl::check_and_update_common_device(common_device, {arg.name}, "{method_name}", "{arg.name}");"""
    ...
    def gen_out_inplace_wrapper(
        self, f: NativeFunction, g: Union[NativeFunctionsGroup, None]
    ) -> Union[str, None]:
        if g is None:
            return None
        k = f.func.kind()
...
"""
    ...
    def gen_unstructured(
        self, f: NativeFunction, g: Union[NativeFunctionsGroup, None] = None
    ) -> Union[str, None]:
...
{create_proxy}"""
    ...
    @method_with_native_function
    def gen_one(self, f: NativeFunction) -> Union[str, None]:
        if f.manual_kernel_registration:
            raise AssertionError(
