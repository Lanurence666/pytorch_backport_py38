...
from typing import Any, Dict, Iterable, Iterator, List, Mapping, Optional, Sequence, Set, TYPE_CHECKING, Tuple, Type, Union, cast, no_type_check, overload
...
def _unflatten_communicated_optim_state(
    ...
    for _ in range(num_unflat_params):
        ...
        for state_name, flat_tensor in sorted_items(tensor_state):
            ...
            optim_state: Union[Union[torch.Tensor, ShardedTensor], DTensor] = next(views)
            if shard_state:
...
def _rekey_sharded_optim_state_dict(
    ...
    optim_input: List[Dict[str, Any]] | Optional[Iterable[nn.Parameter]],
...
def _get_param_id_to_param_from_optim_input(
    model: nn.Module,
    optim_input: List[Dict[str, Any]] | Optional[Iterable[nn.Parameter]] = None,
...
def _map_param_key_to_optim_keys(
    ...
    if merge_keys:
        ...
        key_obj_list: Optional[List[List[_OptimStateKey]]] = (
...
def _convert_state_with_orig_params(
    ...
    for optim_state_key in all_optim_state_keys:
        param_key: Optional[Union[str, int]] = optim_state_key_to_param_key.get(optim_state_key)
        ...
        if optim_state_key.is_fsdp_managed:
            ...
            with SimpleProfiler.profile("none_fsdp_managed_copy"):
                param_key = Union[cast(str, int, param_key)]
