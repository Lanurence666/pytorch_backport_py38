...
def run(n, stmt, fuzzer_cls) -> None:
    ...
    raw_results = []
    for i, (float_values, int_values) in enumerate(_zip_strict(float_iter, double_iter)):
    ...
    for results, spacer in [(parsed_results[:10], "..."), (parsed_results[-10:], "")]:
        for t_float, t_int, rel_diff, descriptions in results:
            ...
            for t_str, (name, shape, sparse_dim, is_coalesced) in _zip_strict(time_str, descriptions):
