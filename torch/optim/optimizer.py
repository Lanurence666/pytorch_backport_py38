...
from typing import Any, Callable, Dict, Iterable, List, Optional, Sequence, Set, Tuple, Type, TypeVar, Union, cast, overload
try:
...
def _disable_dynamo_if_unsupported(
    single_tensor_fn: Callable[..., Optional[object]] = None
...
class Optimizer:
    ...
    def load_state_dict(self, state_dict: StateDict) -> None:
        ...
        if any(
            p_len != s_len for p_len, s_len in _zip_strict(param_lens, saved_lens)
        ):
            raise ValueError(
        ...
        param_groups = [
            update_group(g, ng) for g, ng in _zip_strict(groups, saved_groups)
        ]
