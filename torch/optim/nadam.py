    ...
    *,
    ...
    if not torch.compiler.is_compiling() and capturable:
        ...
        if not all(
            ...
            for p, mp, step in _zip_strict(params, mu_products, state_steps)
        ):
            raise AssertionError(
                "If capturable=True, "
        ...
        if capturable:
            ...
            step_size_grads = _stack_if_compiling(
                [
                    ...
                    for mu_product, mu in _zip_strict(grouped_mu_products, mus)
                ]
            )
            step_size_expavg = _stack_if_compiling(
                [
                    ...
                    * -1
                    for mu_product, mu_next in _zip_strict(
                        grouped_mu_products, mu_nexts)
                ]
            )
