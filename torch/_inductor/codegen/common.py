...
from typing import Any, Callable, ClassVar, Dict, Generic, Iterator, List, MutableMapping, Optional, Sequence, Set, TYPE_CHECKING, Tuple, Type, Union, cast, overload
...
def get_backend_features(
    device: Union[torch.device, Optional[str]],
...
def has_backend_feature(
    device: Optional[Union[torch.device, str]], feature: BackendFeature
) -> bool:
...
class DataTypePropagation:
    def __init__(self, body: LoopBody) -> None:
        self.body = body
        self.graphs: Union[Dict[Callable[..., Any], str], Any] = {
...
class OpOverrides(BasicMathOpsMixin, OpDecompositions, OpsHandler[Any]):
    ...
    @staticmethod
    def constant(value: Union[Union[bool, float], int], dtype: torch.dtype) -> OpVarT:
        return repr(value)
...
class KernelArgs:
    ...
    def _lookup(
        prefix: str,
        odict: Union[Dict[_T, str, RemovedArg], Dict[_T, str]],
        name: _T,
    ) -> str:
...
class CSE(Generic[CSEVariableType, AugmentedKeyT]):
    ...
    def generate(
        ...
        expr: Union[Union[str, CSEVariable], Union[OpsValue, IndentedBuffer]] | DeferredLineBase,
        *,
...
class Kernel(CodeGen, Generic[CSEVariableType]):
    ...
    def __init__(
        ...
        self._load_other: Union[Optional[int], float] = None
...
class KernelTemplate:
    ...
    @property
    def src_hash(self) -> Optional[str]:
        """
        ...
        if var.bounds.lower < 0:
            ...
            if var.bounds != ValueRanges.unknown() and isinstance(size, sympy.Number):
                ...
                if var.bounds.upper >= 0:
                    ...
                    new_bounds = Union[new_bounds, pos]
