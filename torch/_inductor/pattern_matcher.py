...
from typing import Any, Callable, Dict, Generator, Iterable, List, Mapping, NoReturn, Optional, Sequence, Set, Tuple, Type, TypeVar, Union, cast, overload
from typing_extensions import Protocol, Self, TypeIs
...
_debug_nodes_cache: Union[bool, Optional[OrderedSet[str]]] = None
_debug_nodes_env_value_cache: Optional[str] = None


def _should_debug_node(node_name: str) -> bool:
    def _get_debug_nodes() -> Union[bool, OrderedSet[str]]:
        global _debug_nodes_cache, _debug_nodes_env_value_cache

        def parse_debug_env(env_value: Optional[str]) -> Union[bool, OrderedSet[str]]:
            if not env_value:
                return False
            if env_value == "all":
                return True
...
class _TargetArgsExpr(_TargetExpr):
    ...
    def __init__(
        self,
        fns: Union[Union[torch.fx.node.Target, str], Sequence[Any]],
        *args: Any,
...
class PatternEntry:
    ...
    def register(
        ...
        target: Optional[torch.fx.node.Target] = None,
        prepend: bool = False,
    ) -> None:
        if target is None:
...
class ReplacementPatternEntry(PatternEntry):
    ...
    def replace_with_graph(
        ...
        with graph.inserting_before(last_node):
            ...
            def replace(
                old: Optional[torch.fx.Node],
                new: Union[torch.fx.Node, Optional[Sequence[torch.fx.Node]]],
            ) -> None:
...
def register_replacement(
    ...
    example_inputs: Union[List[Any], Tuple[Any, ...]],
    trace_fn: TraceFn,
    pass_dicts: Union[_PassDictsType, Sequence[_PassDictsType]],
    extra_check: Callable[[Match], bool] = _return_true,
    scalar_workaround: Union[Dict[str, float, int], None] = None,
...
def _serialize_pattern(
    ...
    trace_fn: TraceFn,
    scalar_workaround: Union[Dict[str, float, int], None],
) -> PatternExpr:
...
def fx_to_pattern(
    ...
    ) -> Union[Union[T, KeywordArg], Ignored]:
