...
class IntSpec:
    ...
    _min: Optional[int]
    _max: Optional[int]
    _guarding_hint: Optional[int]
    _optimization_hint: Optional[int]
    ...
    def __init__(
        ...
        *,
        ...
        guarding_hint: Optional[int] = None,
        optimization_hint: Optional[int] = None,
    ) -> None:
    ...
    @classmethod
    def static(cls, name: Optional[str] = None, *, value: Optional[int] = None) -> "IntSpec":
...
class TensorSpec:
    ...
    A list-like container of ``Optional[IntSpec]`` with length equal to the
    ...
    - ``list`` / ``tuple`` of ``Optional[IntSpec]`` — dim is inferred from
      length, entries used as-is.
    - ``Dict[int, Optional[IntSpec]]`` — sparse per-dim spec; dim is inferred
    ...
    def __init__(
        self,
        arg: int
        | List[Optional[IntSpec]]
        | Tuple[Optional[IntSpec], ...]
        | Dict[int, Optional[IntSpec]],
    ) -> None:
        if isinstance(arg, int):
            self._dim = arg
            self._specs: List[Optional[IntSpec]] = [None] * arg
    ...
    def __getitem__(self, index: int) -> Optional[IntSpec]:
        return self._specs[index]

    def __setitem__(self, index: int, spec: Optional[IntSpec]) -> None:
        self._specs[index] = spec

    def __len__(self) -> int:
        return self._dim

    def __iter__(self) -> Iterator[Optional[IntSpec]]:
