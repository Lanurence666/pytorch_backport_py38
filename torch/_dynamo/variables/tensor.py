...
from typing import Any, Dict, Iterable, List, NoReturn, Optional, Sequence, Set, TYPE_CHECKING, Tuple, Type, Union, cast, overload
...
class TensorVariable(VariableTracker):
    ...
    def synchronize_attributes(
        self, tx: "InstructionTranslator", target_cls: Optional[type] = None
    ) -> None:
    ...
    def _get_fake_version(self) -> Optional[int]:
    ...
    def method_attr_dtype(self, tx: "InstructionTranslator") -> Optional[VariableTracker]:
        if self.dtype is not None:
            return VariableTracker.build(tx, self.dtype)
        return None

    def method_attr_device(self, tx: "InstructionTranslator") -> Optional[VariableTracker]:
        if self.device is not None:
            return VariableTracker.build(tx, self.device)
        return None

    def method_attr_layout(self, tx: "InstructionTranslator") -> Optional[VariableTracker]:
    ...
    def method_attr_grad(self, tx: "InstructionTranslator") -> Optional[VariableTracker]:
    ...
    def var_getattr(self, tx: "InstructionTranslator", name: str) -> VariableTracker:
        ...
        if result is None and name != "grad":
            ...
            def try_generic_attr_handling() -> Optional[VariableTracker]:
                from .builder import wrap_fx_proxy
                from .misc import GetAttrVariable
    ...
    def unpack_var_sequence(
        self, tx: "InstructionTranslator", idxes: Optional[Sequence[int]] = None
    ...
    def _method_size_stride(
        self, name: str, dim: Optional[Any] = None
    ) -> Optional[VariableTracker]:
    ...
    def method_numel(self, tx: "InstructionTranslator") -> Optional[VariableTracker]:
        if self.valid_size():
    ...
    def method_dim(self, tx: "InstructionTranslator") -> Optional[VariableTracker]:
        if self.ndim is not None:
            return VariableTracker.build(tx, self.ndim)
        return None
    ...
    def method_is_complex(self, tx: "InstructionTranslator") -> Optional[ConstantVariable]:
        if self.dtype is not None:
            return VariableTracker.build(tx, self.dtype.is_complex)
        return None

    def method_is_contiguous(
        self, tx: "InstructionTranslator", memory_format: Optional[VariableTracker] = None
    ) -> Optional[ConstantVariable]:
        memory_format_const = (
    ...
    def method_get_device(self, tx: "InstructionTranslator") -> Optional[VariableTracker]:
    ...
    def method_numpy(
        self, tx: "InstructionTranslator", *, force: Union[VariableTracker, bool] = False
    ...
    def method_tolist(self, tx: "InstructionTranslator") -> VariableTracker:
        ...
        def tolist(tensor: torch.Tensor, sub_proxy: torch.fx.Proxy) -> Union[Any, List[Any]]:
    ...
    def method___setitem__(
        ...
        with torch._dynamo.utils._disable_saved_tensors_hooks_during_tracing(), tx.fake_mode.shape_env.ignore_fresh_unbacked_symbols() if tx.fake_mode and tx.fake_mode.shape_env else nullcontext():
    ...
    def method_add_(
        ...
        *,
        alpha: Optional[VariableTracker] = None,
    ) -> Optional[VariableTracker]:
    ...
    def method_addcdiv_(
        ...
        *,
        value: Optional[VariableTracker] = None,
    ) -> Optional[VariableTracker]:
        if value is not None and config.enable_dynamo_decompositions:
    ...
    def method_requires_grad_(
        self, tx: "InstructionTranslator", requires_grad: Union[bool, VariableTracker] = True
    ) -> VariableTracker:
...
class SymNodeVariable(VariableTracker):
    ...
    def __init__(self, proxy: Any, sym_num: Any, **kwargs: Any) -> None:
        ...
        self._tensor_var: Optional[TensorVariable] = None
...
class UnspecializedPythonVariable(TensorVariable):
    ...
    def from_tensor_variable(
        ...
        raw_value: Union[float, Optional[int]],
        need_unwrap: bool = True,
