...
if(INTERN_BUILD_ATEN_OPS)
    ...
    function(_BUILD_FOR_ADDITIONAL_ARCHS file archs)
      ...
      foreach(_arch ${archs})
        if("${_arch}" STREQUAL "89")
          if(_existing_arch_flags MATCHES ".*compute_86.*" AND CUDA_VERSION VERSION_GREATER_EQUAL "11.8")
            list(APPEND _file_compile_flags "-gencode;arch=compute_89,code=sm_89")
          endif()
        endif()
        if("${_arch}" STREQUAL "90a")
          if(_existing_arch_flags MATCHES ".*compute_90.*" AND CUDA_VERSION VERSION_GREATER_EQUAL "12.0")
            list(APPEND _file_compile_flags "-gencode;arch=compute_90a,code=sm_90a")
          endif()
        endif()
        if("${_arch}" STREQUAL "100a")
          if(_existing_arch_flags MATCHES ".*compute_100.*" AND CUDA_VERSION VERSION_GREATER_EQUAL "12.4")
  ...
  foreach(i RANGE ${NUM_CPU_CAPABILITY_NAMES})
    function(process_vec NAME)
      ...
      if(MSVC AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS 19.30)
        if(("${NAME}" STREQUAL "native/cpu/IndexKernel.cpp") OR ("${NAME}" STREQUAL "native/cpu/LinearAlgebraKernel.cpp"))
          string(REPLACE "/O2" "/Od" FLAGS "${FLAGS}")
          string(APPEND EXTRA_FLAGS " /Od /d2FH4-")
        endif()
      endif()
