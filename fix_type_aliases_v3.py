...
def node_to_str(node):
    if isinstance(node, ast.Name):
        ...
        return f"{node_to_str(node.value)}[{node_to_str(node.slice)}]"
    elif isinstance(node, ast.Index):
        return node_to_str(node.value)
    elif isinstance(node, ast.Tuple):
        return ", ".join(node_to_str(elt) for elt in node.elts)
    elif isinstance(node, ast.List):
        return "[" + ", ".join(node_to_str(elt) for elt in node.elts) + "]"
    elif isinstance(node, ast.Call):
        func_str = node_to_str(node.func)
        ...
        for k, v in zip(node.keys, node.values):
            pairs.append(f"{node_to_str(k)}: {node_to_str(v)}")
        return "{" + ", ".join(pairs) + "}"
    elif isinstance(node, ast.Starred):
        return f"*{node_to_str(node.value)}"
    else:
        return "UNKNOWN"
